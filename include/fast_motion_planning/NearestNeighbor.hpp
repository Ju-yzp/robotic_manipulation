...
private:
    ...
    void update(KdTreeNode* node, bool allow_rebuild) {
        if (!node) return;

        KdTreeNode* parent = node->parent;
        while (parent) {
            parent->tree_size += 1;
            if (allow_rebuild && criterionCheck(parent)) rebuild(&parent);
            parent = parent->parent;
        }
    }

    // 添加点至kd-tree中
    void add_point(Data data, KdTreeNode* root) {
        if (!root) return;
        ...
        while (parent) {
            if (rebuild_tree_ && *rebuild_tree_ == parent) {
                ...
                return;  // 如果是把数据放入缓存队列中给后台重构建树进程，插入新的树中，就直接退出函数
            } else {
    ...
    void mutilRebuild() {
        while (!termination_flag_.load(std::memory_order_acquire)) {
            ...
            if (rebuild_tree_) {
                ...
                KdTreeNode** new_root_node{nullptr};            // 记录新的点
                ...
                while (!pending_queue_.empty()) {
                    ...
                    pending_queue_.pop();
                    ++num;
                    add_point(data, *new_root_node);
                    rebuild_flag_.clear(std::memory_order_release);
                    if (num % 40 == 0) std::this_thread::sleep_for(std::chrono::microseconds(50));
                    while (rebuild_flag_.test_and_set(std::memory_order_acquire))
                        ;
                }
                rebuild_flag_.clear(std::memory_order_release);
    ...
    void search(PointType point, Data& nearest_node) {
        ...
        //         continue;
        //     else if (search_state_counter_.compare_exchange_strong(
        //                  REBUILDING_FLAG, 0, std::memory_order_acq_rel,
        //                  std::memory_order_acquire)) {
        //         search_by_tree(rebuilding_tree, point, nearest_node);
    ...
    // 搜索以root为根节点的树
    // void search_by_tree(KdTreeNode* root, PointType point, Data& nearest_node) {
    //     std::stack<KdTreeNode*> ns;
    //     ns.emplace(root);
    ...
    //         double cur_dist_sqr = calc_center_dist(node, point);
    //         double radius_sqr = calc_dist(nearest_node.point, point);
    //         if (radius_sqr + node->radius_sqr < cur_dist_sqr) continue;

    //         if (node->right_child) ns.emplace(node->right_child);
    //         if (node->left_child) ns.emplace(node->left_child);
    //         double dist = calc_dist(node->data.point, point);

    //         if (dist > radius_sqr) continue;
    //         nearest_node = node->data;
    //     }
    // }
    ...
    bool criterionCheck(KdTreeNode* root) {
        ...
        KdTreeNode* child_ptr = root->left_child;
        if (!child_ptr) child_ptr = root->right_child;
    ...
    void rebuild(KdTreeNode** root) {
        ...
        if ((*root)->tree_size >= mutil_rebuild_point_num_) {
            ...
            if (rebuild_lock.owns_lock())
                if (rebuild_tree_ == nullptr || (*root)->tree_size > (*rebuild_tree_)->tree_size)
                    rebuild_tree_ = root;
        } else {
            parent = (*root)->parent;
            pcl_storage_.clear();

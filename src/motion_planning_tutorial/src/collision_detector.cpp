...
bool CollisionDetector::isOccurCollision(const Eigen::Vector4d& center, const double radius) {
    ...
    for (int i{0}; i < scene_.obstacle_centers.size(); ++i) {
        // std::cout<<"wow"<<std::endl;
        double distance = (scene_.obstacle_centers[i] - center).norm();
        if (distance < radius + scene_.obstacle_radius[i]) {
            return true;  // 发生碰撞
        }
    }

    // std::cout<<"No occur collision"<<std::endl;
    return false;
}
}  // namespace motion_planning_tutorial

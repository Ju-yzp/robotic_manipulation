...
std::unordered_map<std::size_t, Eigen::Matrix4d> Ur5eKinematicSolver::forwardKinematic(
    ...
    assert(state.size() == dof_ && "State positions size must be 6 for Ur5eKinematic");

    Eigen::Matrix4d transform_matrix = Eigen::Matrix4d::Identity();
    for (size_t id{0}; id < dof_; id++) {
        transform_matrix = transform_matrix *
                           frameTransform(a_table_[id], d_table_[id], alpha_table_[id], state[id]);
        tf_map[id] = transform_matrix;
    }
    return tf_map;
}

std::vector<State> Ur5eKinematicSolver::inverseKinematic(const Eigen::Matrix4d end_effector_pose) {
    ...
    Eigen::Matrix4d frame6_to_frame0 = end_effector_pose * endeffector_to_frame6.inverse();
    ...
    for (size_t id{0}; id < all_solutions.size(); id++) {
        ...
        for (size_t joint_id{0}; joint_id < dof_; joint_id++) {
            state[joint_id] = all_solutions[id][joint_id];
        }
...
bool Ur5eKinematicSolver::getArmThetas(
...
Eigen::Matrix4d Ur5eKinematicSolver::get_endeffector_pose(const State& state) {
    Eigen::Matrix4d endeffector_pose = Eigen::Matrix4d::Identity();
